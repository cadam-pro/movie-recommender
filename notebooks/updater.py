...
from pyspark.sql.functions import expr, lit, col, coalesce, udf
...
schema = StructType(
    [
        ...
        StructField(
            "genres", ArrayType(StructType([StructField("name", StringType(), True)]))
        ),
        ...
        StructField(
            ...
            ArrayType(StructType([StructField("name", StringType(), True)])),
        ),
        StructField(
        ...
        StructField(
            "spoken_languages",
            ArrayType(StructType([StructField("english_name", StringType(), True)])),
        ),
...
def get_last_movies_change():
    ...
    for page in range(1, 500):
        data = call_api(CHANGE_URL, {"page": page})
        if data["results"] != []:
            id_movies += [
                movie["id"]
                for movie in data["results"]
                if (not movie["adult"]) and (movie["adult"] is not None)
            ]
        else:
            break
    return id_movies
...
def update_release_date(change):
    if "value" in change:
        return datetime.strptime(
            change.get("value").get("release_date", {}), "%Y-%m-%d"
        ).date()
    else:
        return datetime.strptime(
            change.get("original_value").get("release_date", {}), "%Y-%m-%d"
        ).date()
                ...
                if change.get("key") != "status":
                    if (change.get("key") in columns) or (change.get("key") == "crew"):
                        ...
                        match change.get("key"):
                            case "release_dates":
                                row[columns.index(change.get("key"))] = (
                                    update_release_date(change.get("items")[0])
                                )
                            case "tagline":
                                row[columns.index(change.get("key"))] = update_tagline(
                                    change.get("items")
                                )
                            case "cast":
                                if credits == []:
                            ...
                            case "production_companies":
                                row[columns.index(change.get("key"))] = (
                                    update_production_company(change.get("items"))
                                )
                            case "genres":
                                row[columns.index(change.get("key"))] = update_genre(
                                    change.get("items")
                                )
                            case _:
...
def apply_update(df_movie, update_df):
    ...
    df_merged = joined.select(
        col("id"), *[coalesce(col(f"b.{c}"), col(f"a.{c}")).alias(c) for c in columns]
    )
    return df_merged
...
new_movies_ids = list(
    set(retrieve_tmdb_movie_id(today_str)) - set(retrieve_tmdb_movie_id(yesterday_str))
)
...
def get_movie(id=1):
    data = call_api(
        GET_MOVIE_URL + str(id),
        {"language": "en-EN", "append_to_response": "credits", "id": id},
    )
    return data
...
def map_movies(data):
    ...
    movieDF = movieDF.withColumn(
        "genres", expr("array_join(transform(genres, x -> x.name), ', ')")
    )
    movieDF = movieDF.withColumn(
        "production_countries",
        expr("array_join(transform(production_countries, x -> x.name), ', ')"),
    )
    movieDF = movieDF.withColumn(
        "production_companies",
        expr("array_join(transform(production_companies, x -> x.name), ', ')"),
    )
    movieDF = movieDF.withColumn(
        "spoken_languages",
        expr("array_join(transform(spoken_languages, x -> x.english_name), ', ')"),
    )

    movieDF = movieDF.withColumn(
        "director", map_crewUDF(sf.to_json(movieDF.credits), lit(["Director"]))
    )
    movieDF = movieDF.withColumn(
        "director_of_photography",
        map_crewUDF(sf.to_json(movieDF.credits), lit(["Director of Photography"])),
    )
    movieDF = movieDF.withColumn(
        "writers", map_crewUDF(sf.to_json(movieDF.credits), lit(["Writer"]))
    )
    movieDF = movieDF.withColumn(
        "producers",
        map_crewUDF(
            sf.to_json(movieDF.credits), lit(["Producer", "Executive Producer"])
        ),
    )
    movieDF = movieDF.withColumn(
        "music_composer",
        map_crewUDF(sf.to_json(movieDF.credits), lit(["Original Music Composer"])),
    )
...
imdb_df = spark1.read.csv(
    "./title.ratings.tsv", header=True, inferSchema=True, sep="\t"
)
...
df_new_movies_with_imdb_rating = df_new_movies_with_imdb_rating.withColumn(
    "release_date", df_new_movies_with_imdb_rating["release_date"].try_cast("date")
)
